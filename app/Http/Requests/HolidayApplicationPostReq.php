...
class HolidayApplicationPostReq extends FormRequest
{
    ...
    {
        return [
            'holiday_type_id' => 'required|max:10',
    ...
    {
        $validator->sometimes('holiday_date_to', 'required|date|after_or_equal:holiday_date_from', function ($input) {
            return $input->holiday_type_id == 1 || $input->holiday_type_id == 3;
        });
        $validator->sometimes('holiday_time_from', 'required|date_format:H:i', function ($input) {
            return $input->holiday_type_id == 2;
        });
    ...
    public function messages()
    {
        return [
            ...
            'holiday_date_to.after_or_equal' => '休暇開始日以降の日を選択してください。',
